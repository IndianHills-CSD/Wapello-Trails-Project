<!Doctype html>
<html lang="en">
	<head>
	   <meta charset="UTF-8">
	   <meta name="viewport" content="width=device-width, initial-scale=1.0">
	   <title> Gallery Page </title>
		<link  rel="stylesheet" type="text/css" href="css/gallery-style.css">
	    <link  rel="stylesheet" type="text/css" href="css/Style.php">
		<script src="js/slide-img.js"></script>
	</head>
	<body>
		<?php include 'nav.php';?>

		<!-- Container for the image gallery -->
		<main class="body-container">
			<h1 style="text-align:center"><strong>Wapello County Trails Council Gallery</strong></h1>

			<div class="ContainerBtn">
				<button class="btn active" data-class="all"> Show All</button>
				<button class="btn" data-class="videos"> Videos</button>
				<button class="btn" data-class="chief"> Chief Wapello</button>
				<button class="btn" data-class="Volunteers"> Volunteers</button>
				<button class="btn" data-class="wabash"> Wabash Bridge</button>
				<button class="btn" data-class="trails"> Trails & Nature</button>
			</div>

			<div class="Video-container">
				<div class="item" data-class="videos">
					<h2 style="text-align:center"><strong>Chief Wapello Carving and Wabash Video</strong></h2>
					<iframe width="550" height="415"
						src="https://www.youtube.com/embed/-W1m8Kw5UHE" frameborder="0"
						allow="accelerometer; autoplay; encrypted-media;
						gyroscope; picture-in-picture" allowfullscreen>
					</iframe>

					<iframe width="550" height="415"
						src="https://www.youtube.com/embed/kqGcuXj-rMk" frameborder="0"
						allow="accelerometer; autoplay; encrypted-media; gyroscope;
						picture-in-picture" allowfullscreen>
					</iframe>
				</div>
			</div>

			<h2 style="text-align:center"><strong>Photos</strong></h2>
			<div class="flex-img">
				<div class="item" data-class="chief">
					<img src="images/img-01.jpg" onclick="openModal(); currentSlide(01)">
				</div>
				<div class="item" data-class="chief">
					<img src="images/img-02.jpg"  onclick="openModal();currentSlide(02)">
				</div>
				<div class="item" data-class="chief">
					<img src="images/img-03.jpg"  onclick="openModal(); currentSlide(03)">
				</div>
				<div class="item" data-class="Volunteers">
					<img src="images/img04.jpg"  onclick="openModal();currentSlide(04)">
				</div>
				<div class="item" data-class="Volunteers">
					<img src="images/img05.jpg"  onclick="openModal();currentSlide(05)">
				</div>
				<div class="item" data-class="Volunteers">
					<img src="images/img06.jpg"  onclick="openModal();currentSlide(06)">
				</div>
				<div class="item" data-class="wabash">
					<img src="images/img07.jpg"  onclick="openModal();currentSlide(07)">
				</div>
				<div class="item">
					<img src="images/img08.jpg"  onclick="openModal();currentSlide(08)">
				</div>
				<div class="item" data-class="wabash">
					<img src="images/img09.jpg"  onclick="openModal();currentSlide(09)">
				</div>
				<div class="item" data-class="wabash">
					<img src="images/img12.jpg"  onclick="openModal();currentSlide(10)">
				</div>
				<div class="item" data-class="wabash">
					<img src="images/trail-site.jpg" onclick="openModal(); currentSlide(11)">
				</div>
				<div class="item">
					<img src="images/trail-site02.jpg"  onclick="openModal();currentSlide(12)">
				</div>
				<div class="item">
					<img src="images/trail-site03.jpg"  onclick="openModal(); currentSlide(13)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site18.jpg"  onclick="openModal();currentSlide(14)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site01.jpg" onclick="openModal(); currentSlide(15)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site05.jpg"  onclick="openModal(); currentSlide(16)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site19.jpg"  onclick="openModal();currentSlide(17)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site06.jpg" onclick="openModal(); currentSlide(18)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site07.jpg"  onclick="openModal();currentSlide(19)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site08.jpg"  onclick="openModal(); currentSlide(20)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site17.jpg"  onclick="openModal();currentSlide(21)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site09.jpg" onclick="openModal(); currentSlide(22)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site10.jpg"  onclick="openModal();currentSlide(23)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site11.jpg"  onclick="openModal(); currentSlide(24)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site16.jpg"  onclick="openModal();currentSlide(25)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site12.jpg" onclick="openModal(); currentSlide(26)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site13.jpg"  onclick="openModal();currentSlide(27)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site14.jpg"  onclick="openModal(); currentSlide(28)" >
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site15.jpg"  onclick="openModal();currentSlide(29)">
				</div>
				<div class="item" data-class="wabash">
					<img src="images/trail-site20.jpg"  onclick="openModal();currentSlide(30)">
				</div>

				<div class="item" data-class="trails">
					<img src="images/trail-site21.jpg" onclick="openModal(); currentSlide(31)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site22.jpg"  onclick="openModal();currentSlide(32)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site23.jpg"  onclick="openModal(); currentSlide(33)" >
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site24.jpg"  onclick="openModal();currentSlide(34)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site25.jpg" onclick="openModal(); currentSlide(35)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site26.jpg"  onclick="openModal(); currentSlide(36)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site27.jpg"  onclick="openModal();currentSlide(37)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site28.jpg" onclick="openModal(); currentSlide(38)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site29.jpg"  onclick="openModal();currentSlide(39)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site30.jpg"  onclick="openModal(); currentSlide(40)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site31.jpg"  onclick="openModal();currentSlide(41)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site32.jpg" onclick="openModal(); currentSlide(42)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site33.jpg"  onclick="openModal();currentSlide(43)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site34.jpg"  onclick="openModal(); currentSlide(44)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site35.jpg"  onclick="openModal();currentSlide(45)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site36.jpg" onclick="openModal(); currentSlide(46)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site37.jpg"  onclick="openModal();currentSlide(47)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site38.jpg"  onclick="openModal(); currentSlide(48)">
				</div>
				<div class="item" data-class="wabash">
					<img src="images/trail-site39.jpg"  onclick="openModal();currentSlide(49)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site40.jpg"  onclick="openModal();currentSlide(50)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site41.jpg" onclick="openModal(); currentSlide(51)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site42.jpg"  onclick="openModal();currentSlide(52)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site43.jpg"  onclick="openModal(); currentSlide(53)">
				</div>
				<div class="item" data-class="trails">
					<img src="images/trail-site44.jpg"  onclick="openModal();currentSlide(54)">
				</div>
			</div>


			<div id="myModal" class="modal">
				<span class="close cursor" onclick="closeModal()">&times;</span>
				<div class="slider">
					<div class="mySlides">
  				  <div class="numbertext">1 / 60</div>
  				  <img src="images/img-01.jpg" style="width:100%">
  				   <div class="caption">Chief Wapello curving photo</div>
  				</div>
  				<div class="mySlides">
  				  <div class="numbertext">2 / 60</div>
  				  <img src="images/img-02.jpg" style="width:100%">
  				   <div class="caption">Chief Wapello curving photo</div>
  				</div>

  				<div class="mySlides">
  				  <div class="numbertext">3 / 60</div>
  				  <img src="images/img-03.jpg" style="width:100%">
  				   <div class="caption">Chief Wapello curving photo</div>
  				</div>

  				<div class="mySlides">
  				  <div class="numbertext">4 / 60</div>
  				  <img src="images/img04.jpg" style="width:100%">
  				   <div class="caption">Volunteer Working on the Wabash Bridge</div>
  				</div>

  				<div class="mySlides">
  				  <div class="numbertext">5 / 60</div>
  				  <img src="images/img05.jpg" style="width:100%">
  				   <div class="caption">Volunteers on the Trail Bridge</div>
  				</div>

  				<div class="mySlides">
  				  <div class="numbertext">6 / 60</div>
  				  <img src="images/img06.jpg" style="width:100%">
  				   <div class="caption">Volunteers</div>
  				</div>

  				<div class="mySlides">
  				  <div class="numbertext">7 / 60</div>
  				  <img src="images/img07.jpg" style="width:100%">
  				   <div class="caption"> Wabash Bridge</div>
  				</div>

  				<div class="mySlides">
  				  <div class="numbertext">8 / 60</div>
  				  <img src="images/img08.jpg" style="width:100%">
  				   <div class="caption">Resting Area </div>
  				</div>

  				<div class="mySlides">
  				  <div class="numbertext">9 / 60</div>
  				  <img src="images/img09.jpg" style="width:100%">
  				   <div class="caption">Resting Seat along the Bridge.</div>
  				</div>

  				<div class="mySlides">
  				  <div class="numbertext">10 / 60</div>
  				  <img src="images/img12.jpg" style="width:100%">
  				   <div class="caption">Wabash Bridge at Night</div>
  				</div>

  				<div class="mySlides">
				  <div class="numbertext">11 / 60</div>
				  <img src="images/trail-site-bigger.jpg" style="width:100%">
				   <div class="caption">Wabash Bridge on Des Moines River</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">12 / 60</div>
				  <img src="images/trail-site02-bigger.jpg" style="width:100%">
				   <div class="caption">Jefferson Street Viaduct</div>
				</div>
				<div class="mySlides">
				  <div class="numbertext">13 / 60</div>
				  <img src="images/trail-site03-bigger.jpg" style="width:100%">
				   <div class="caption">Viaduct Accent Lighting</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">14 / 60</div>
				  <img src="images/trail-site18-bigger.jpg" style="width:100%">
				   <div class="caption">Trail-Path and Green</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">15 / 60</div>
				  <img src="images/trail-site01-bigger.jpg" style="width:100%">
				   <div class="caption">Trail Walk towards Wabash Bridge on Des Moines</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">16 / 60</div>
				  <img src="images/trail-site05-bigger.jpg" style="width:100%">
				   <div class="caption">Trail-Path under Wabash Bridge</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">17 / 60</div>
				  <img src="images/trail-site19-bigger.jpg" style="width:100%">
				   <div class="caption">Pathway with Green</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">18 / 60</div>
				  <img src="images/trail-site06-bigger.jpg" style="width:100%">
				   <div class="caption">Trails around the park</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">19 / 30</div>
				  <img src="images/trail-site07-bigger.jpg" style="width:100%">
				   <div class="caption"> Wabash Bridge on Des Moines River View</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">20/ 60</div>
				  <img src="images/trail-site08-bigger.jpg" style="width:100%">
				   <div class="caption"> Bridge across Ottumwa Waters</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">21 / 60</div>
				  <img src="images/trail-site17-bigger.jpg" style="width:100%">
				   <div class="caption">Trail Walk in Green</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">22 / 60</div>
				  <img src="images/trail-site09-bigger.jpg" style="width:100%">
				   <div class="caption"> Sunset on Des Moines </div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">23 / 60</div>
				  <img src="images/trail-site10-bigger.jpg" style="width:100%">
				   <div class="caption">Resting Area on Des Moines River</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">24 / 60</div>
				  <img src="images/trail-site11-bigger.jpg" style="width:100%">
				   <div class="caption">Chief Wapello Trail Entry View</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">25 / 60</div>
				  <img src="images/trail-site16-bigger.jpg" style="width:100%">
				   <div class="caption">Trail Walk to Wabash Bridge</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">26 / 30</div>
				  <img src="images/trail-site12-bigger.jpg" style="width:100%">
				   <div class="caption">Entry to Chief Wapello Trail on Des Moines River</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">27 / 60</div>
				  <img src="images/trail-site13-bigger.jpg" style="width:100%">
				   <div class="caption">Woods Pathway</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">28 / 60</div>
				  <img src="images/trail-site14-bigger.jpg" style="width:100%">
				   <div class="caption">Trail Walk and Resting Area</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">29 / 60</div>
				  <img src="images/trail-site15-bigger.jpg" style="width:100%">
				   <div class="caption">Trail Walk</div>
				</div>

				<div class="mySlides">
				  <div class="numbertext">30 / 60</div>
				  <img src="images/trail-site20-bigger.jpg" style="width:100%">
				   <div class="caption">Trail Walk under the Bridge</div>
				</div>

					<div class="mySlides">
					  <div class="numbertext">31/ 60</div>
					  <img src="images/trail-site21-bigger.jpg" style="width:100%">
					   <div class="caption">Trail path</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">32 / 60</div>
					  <img src="images/trail-site22-bigger.jpg" style="width:100%">
					   <div class="caption">Green on River</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">33 / 60</div>
					  <img src="images/trail-site23-bigger.jpg" style="width:100%">
					   <div class="caption">Walk To Wabash Bridge</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">34 / 60</div>
					  <img src="images/trail-site24-bigger.jpg" style="width:100%">
					   <div class="caption">Trail Path under Wabash Bridge</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">35 / 60</div>
					  <img src="images/trail-site25-bigger.jpg" style="width:100%">
					   <div class="caption">Path to the Woods</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">36 / 60</div>
					  <img src="images/trail-site26-bigger.jpg" style="width:100%">
					   <div class="caption">Path to the Woods</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">37 / 60</div>
					  <img src="images/trail-site27-bigger.jpg" style="width:100%">
					   <div class="caption">Trails beside Des Moines River</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">38 / 60</div>
					  <img src="images/trail-site28-bigger.jpg" style="width:100%">
					   <div class="caption">River View</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">39 / 60</div>
					  <img src="images/trail-site29-bigger.jpg" style="width:100%">
					   <div class="caption">Tree Path</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">40 / 60</div>
					  <img src="images/trail-site30-bigger.jpg" style="width:100%">
					   <div class="caption">Bush's on Trail Path</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">41 / 60</div>
					  <img src="images/trail-site31-bigger.jpg" style="width:100%">
					   <div class="caption">River Stream View</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">42 / 60</div>
					  <img src="images/trail-site32-bigger.jpg" style="width:100%">
					   <div class="caption">Trees on River</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">43 / 60</div>
					  <img src="images/trail-site33-bigger.jpg" style="width:100%">
					   <div class="caption">Trail Trees</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">44 / 60</div>
					  <img src="images/trail-site34-bigger.jpg" style="width:100%">
					   <div class="caption">River surface View</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">45 / 60</div>
					  <img src="images/trail-site35-bigger.jpg" style="width:100%">
					   <div class="caption">Des Moines Rivershore</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">46 / 60</div>
					  <img src="images/trail-site36-bigger.jpg" style="width:100%">
					   <div class="caption">Green-Park Trail</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">47 / 50</div>
					  <img src="images/trail-site37-bigger.jpg" style="width:100%">
					   <div class="caption">Bush's Park </div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">48 / 60</div>
					  <img src="images/trail-site38-bigger.jpg" style="width:100%">
					   <div class="caption">Walking in Woods</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">49 / 60</div>
					  <img src="images/trail-site39-bigger.jpg" style="width:100%">
					   <div class="caption">Wabash Bridge on Des Moines River</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">50 / 60</div>
					  <img src="images/trail-site40-bigger.jpg" style="width:100%">
					   <div class="caption">Trail in Bushes</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">51 / 60</div>
					  <img src="images/trail-site41-bigger.jpg" style="width:100%">
					   <div class="caption">Trail between Bushes</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">52 / 60</div>
					  <img src="images/trail-site42-bigger.jpg" style="width:100%">
					   <div class="caption">Wabash Bridge on River</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">53 / 60</div>
					  <img src="images/trail-site43-bigger.jpg" style="width:100%">
					   <div class="caption">River Bushes</div>
					</div>

					<div class="mySlides">
					  <div class="numbertext">54 / 60</div>
					  <img src="images/trail-site44-bigger.jpg" style="width:100%">
					   <div class="caption">Wild Bush River</div>
					</div>

					<!--Previous and Next Button on images slideshow -->
					<a class="prev" onclick="plusSlides(-1)">&#10094;</a>
					<a class="next" onclick="plusSlides(1)">&#10095;</a>

					<!--slideshow images caption-->
					<div class="content-caption">
						<p id="caption"></p>
					</div>

				</div>
			</div>
		</main>

		<?php include 'footer.php';?>

		<script>

			// javaScript for filter
			const listItems = document.querySelectorAll('.ContainerBtn .btn');
			const allimages = document.querySelectorAll('.container .item');

			function toggleActiveClass(active){
				listItems.forEach(item => {
				  item.classList.remove('active');
				})
				active.classList.add('active');
			}

			function toggleimages(dataClass){
				if(dataClass === 'all'){
					for(let i = 0; i<allimages.length; i++){
						allimages[i].style.display = 'block';
					}
				}else{
					for(let i = 0; i<allimages.length; i++)
						allimages[i].dataset.class === dataClass ? allimages[i].style.display = 'block' : allimages[i].style.display = 'none';
				}
			}

			for(let i = 0; i < listItems.length; i++){
				listItems[i].addEventListener('click', function(){
					toggleActiveClass(listItems[i]);
					toggleimages(listItems[i].dataset.class);
				});
			}
		</script>
	</body>
</html>
